...
DOCK_THEME="modern"
if [ -f $HOME/.config/supersection/settings/dock-theme ]; then
    DOCK_THEME=$(cat $HOME/.config/supersection/settings/dock-theme)
fi
echo ":: Using Dock Theme $DOCK_THEME"
echo ":: Dock Autohide $DOCK_AUTOHIDE"
if [ ! -f $HOME/.config/supersection/settings/dock-disabled ]; then
    killall nwg-dock-hyprland
    sleep 0.5
    if [ -f $HOME/.config/supersection/settings/dock-autohide ]; then
