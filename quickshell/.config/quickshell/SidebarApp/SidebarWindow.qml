...
PanelWindow {
    ...
    component SuperSectionButton: Button {
    ...
    component SuperSectionSwitch: Switch {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                ActionIcon {
                    iconTxt: "󰔎"
                    onClicked: {
                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/supersection/scripts/supersection-toggle-theme"])
                    }
                }
                ...
                ActionIcon {
                    ...
                    onClicked: {
                        root.isOpen = false
                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/supersection/settings/hyprpicker.sh"])
                    }
                }
            ...
            RowLayout {
                ...
                SuperSectionButton { 
                    ...
                    onClicked: {
                        root.isOpen = false
                        Quickshell.execDetached(["bash", "-c", "qs -p " + Quickshell.env("HOME") + "/.local/share/supersection-dotfiles-settings/quickshell ipc call settings toggle"])
                    }
                }
                SuperSectionButton { 
                    text: "Hyprland"
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        SuperSectionSwitch { 
                            ...
                            Process {
                                command: ["bash", "-c", "test -f ~/.config/supersection/settings/waybar-disabled && echo 0 || echo 1"]
                                running: root.isOpen 
                                stdout: StdioCollector {
                                    onStreamFinished: {
                            ...
                            onClicked: {
                                if (!ready) return;
                                let fileCmd = checked 
                                ? "rm -f ~/.config/supersection/settings/waybar-disabled"
                                : "touch ~/.config/supersection/settings/waybar-disabled"       
                        ...
                        SettingsWheel {
                            ...
                            Menu {
                                ...
                                SuperSectionMenuItem { text: "Select Waybar Theme"; onClicked: {
                                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/waybar/themeswitcher.sh"])
                                    }
                                }
                                SuperSectionMenuItem { text: "Edit Quicklinks"; onClicked: {
                                        root.isOpen = false
                                        Quickshell.execDetached(["gnome-text-editor", Quickshell.env("HOME") + "/.config/supersection/settings/waybar-quicklinks.json"])
                                    }
                                }
                                SuperSectionMenuItem { text: "Reload Waybar"; onClicked: {
                    ...
                    RowLayout {
                        ...
                        SuperSectionSwitch { 
                            ...
                            property bool ready: false
                            Process {
                                command: ["bash", "-c", "test -f ~/.config/supersection/settings/dock-disabled && echo 0 || echo 1"]
                                running: root.isOpen 
                                stdout: StdioCollector {
                                    onStreamFinished: {
                            ...
                            onClicked: {
                                if (!ready) return;
                                let fileCmd = checked 
                                ? "rm -f ~/.config/supersection/settings/dock-disabled"
                                : "touch ~/.config/supersection/settings/dock-disabled"
                    ...
                    RowLayout {
                        ...
                        SuperSectionSwitch { 
                            ...
                            property bool ready: false
                            Process {
                                command: ["bash", "-c", "test -f ~/.config/supersection/settings/gamemode-enabled && echo 0 || echo 1"]
                                running: root.isOpen 
                                stdout: StdioCollector {
                                    onStreamFinished: {
                        ...
                        SuperSectionSwitch { 
                            ...
                            onClicked: {
                                if (checked) {
                                    ...
                                    Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/supersection/scripts/supersection-sidepad --init"])
                                } else {
                                    console.log("Stopping sidebar...")
                                    Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/supersection/scripts/supersection-sidepad --kill"])
                                }
                            }
                        }
                        SettingsWheel {
                            ...
                            Menu {
                                ...
                                background: Rectangle { color: Theme.background; border.color: Theme.primary; border.width: 1; radius: 8 }
                                SuperSectionMenuItem { text: "Select Sidepad"; onClicked: {
                                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/supersection/scripts/supersection-sidepad --select"])
                                    } 
                                }
                                SuperSectionMenuItem { text: "Open Sidepad Folder"; onClicked: {
                    ...
                    RowLayout {
                        ...
                        ActionIcon { 
                            ...
                            onClicked: {
                                root.isOpen = false
                                Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/supersection/themes/themes.sh"])
                            }
                        }
                        SettingsWheel {
                            ...
                            Menu {
                                ...
                                background: Rectangle { color: Theme.background; border.color: Theme.primary; border.width: 1; radius: 8 }
                                SuperSectionMenuItem { text: "Set GTK Theme"; onClicked: {
                                        root.isOpen = false
                                        Quickshell.execDetached(["nwg-look"])
                                    } 
                                }
                                SuperSectionMenuItem { text: "Set QT Theme"; onClicked: {
                                        root.isOpen = false
                                        Quickshell.execDetached(["qt6ct"])
                                    }
                                }
                                SuperSectionMenuItem { text: "Refresh GTK Theme"; onClicked: {
                                        root.isOpen = false
