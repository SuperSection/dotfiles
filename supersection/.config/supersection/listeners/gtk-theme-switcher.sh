...
apply_theme() {
    ...
    if [[ "$THEME_PREF" == "1" || "$THEME_PREF" == "true" ]]; then
        ...
        if [ -f $HOME/.cargo/bin/matugen ]; then
            $HOME/.cargo/bin/matugen image $(cat ~/.cache/supersection/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "dark"
        else
            matugen image $(cat ~/.cache/supersection/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "dark"
        fi
        ...
        # Update SuperSection Dotfiles Settings theme
        qs -p $HOME/.local/share/supersection-dotfiles-settings/quickshell ipc call theme-manager reload
        echo "SuperSection Dotfiles Settings Theme updated"

        # Reload Waybar
        nohup bash -c "$HOME/.config/waybar/scripts/launch.sh" > /dev/null 2>&1 &
        disown
        ...
        if [ -f $HOME/.cargo/bin/matugen ]; then
            $HOME/.cargo/bin/matugen image $(cat ~/.cache/supersection/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "light"
        else
            matugen image $(cat ~/.cache/supersection/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "light"
        fi
